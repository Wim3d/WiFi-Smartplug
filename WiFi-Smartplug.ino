...
const char* devicename = "BSD33_1"; // 7 characters, then the length of the MQTT topic fits nicely
...
#define DEVICE_TYPE1 1    // Sonoff separate LEDs for status and relay, SonoffBasic R3, Sonoff S20, S26
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if ((char)topic[8] == 'L')      // check for the "L" of sonoff#/LED_sw
  {
    ...
    {
      digitalWrite(LEDPIN, LOW);   // Turn the LED on by smaking the voltage LOW (inversed for this LED)
...
void handle_ledoff() {
  digitalWrite(LEDPIN, HIGH);   // Turn the LED on by smaking the voltage LOW (inversed for this LED)
  client.publish(LED_st_topic, "OFF");
...
String SendHTML() {
  ...
  ptr += "<br></p>";
  //if (ledstatus)
  if (digitalRead(LEDPIN) == LOW) // LED is on (reversed logic)
  {
...
{
  if (DEVICE == DEVICE_TYPE2)   // set LED to Relay state
  {
